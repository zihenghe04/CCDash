...
function statusBadge(status, lastActivity) {
  ...
  const title = lastActivity ? `Last: ${lastActivity.slice(0,19).replace('T',' ')}` : '';
...
function renderSessionsData(d) {
  ...
  document.getElementById('tbSess').innerHTML = cliSessions.map(s =>
    `<tr data-row-source="${s.source||'claude'}"><td style="width:24px;padding:4px"><input type="checkbox" class="sess-check" data-sid="${s.sessionId}" onclick="event.stopPropagation();updateCompareBtn()"></td><td>${statusBadge(s.status, s.lastActivity)}</td><td onclick="showSessionDetail('${s.sessionId}')" style="cursor:pointer" title="${curLang==='zh'?'点击查看详情':'Click for details'}">${fmtT(s.timestamp)}</td><td onclick="showSessionDetail('${s.sessionId}')" style="cursor:pointer" title="${s.project}">${s.projectShort}${epBadge(s.entrypoint)}</td><td onclick="showSessionDetail('${s.sessionId}')" style="cursor:pointer;max-width:200px;overflow:hidden;text-overflow:ellipsis">${s.firstPrompt||'—'}</td></tr>`
  ).join('');
  updateCompareBtn();
